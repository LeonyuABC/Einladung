...
3. 发布本项目附带的 Firestore Rules（新增每日心情后需要重新发布一次）。
...
1. 在 IntelliJ IDEA 打开整个 `couple-space-mood-pwa` 文件夹；
2. 右键 `index.html`；
...
- Yaoyu 和 Daria 分别保存当天心情，检查另一台设备是否自动出现；
...
Firebase 不需要为 PWA 再开任何功能。上传全部新文件并等待 GitHub Pages 部署完成后：

- Android Chrome：菜单 → `安装应用` 或 `添加到主屏幕`；
- iPhone Safari：分享 → `添加到主屏幕`。

如果没有看到安装选项，先刷新公开网址并确认 `manifest.webmanifest`、`service-worker.js` 和 `icons` 文件夹都已经上传。
